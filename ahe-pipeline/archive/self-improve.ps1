...
function Invoke-Discovery {
    ...
    if (Test-Path $ceDir) {
        ...
        if ($missing) {
            ...
            $candidates += [PSCustomObject]@{ Type="CEskill"; Name=$_; Source="CE plugin"; Date=Get-Date }
        }
    }
        ...
        if ($ghSearch -and $ghSearch.items) {
            foreach ($item in $ghSearch.items) {
                ...
                if ($desc -and $stars -gt 10) {
                    ...
                    if (-not $knownNames) {
                        $candidates += [PSCustomObject]@{ Type="GitHubSkill"; Name=$name; Stars=$stars; Description=$desc; Source="GitHub API"; Date=Get-Date }
                    }
                }
            }
        }
        ...
        if ($mcpSearch -and $mcpSearch.items) {
            ...
            foreach ($item in $mcpSearch.items) {
                ...
                if (-not $alreadyHave -and $stars -gt 500) {
                    ...
                    $candidates += [PSCustomObject]@{ Type="MCPCandidate"; Name=$name; Stars=$stars; Source="GitHub API"; Date=Get-Date }
                }
            }
        }
    ...
    try {
        ...
        if ($npmVer -and $npmVer -ne $currentVer) {
            ...
            $candidates += [PSCustomObject]@{ Type="QwenUpdate"; Name="qwen-code"; Current=$currentVer; Latest=$npmVer; Source="npm" }
        } else {
...
function Invoke-Benchmark {
    ...
    Log "Running benchmark-system.ps1 (k=$Runs)..."
    try {
...
function Invoke-Gate {
    ...
    $pass=0;$fail=0
    foreach($s in @("self-improve.ps1","benchmark-system.ps1","agent-debugger.ps1","ahe-backup-rollback.ps1","verify-mcps.ps1","sync-obsidian.ps1")){if(Test-Path "$env:USERPROFILE\Scripts\$s"){$pass++}else{$fail++;Log "GATE MISSING: $s"}}
    ...
    foreach ($c in $allCandidates) {
        ...
        switch ($c.Type) {
            "MCPCandidate" {
                ...
                $atRisk = "Possible env var conflicts or startup failures"
