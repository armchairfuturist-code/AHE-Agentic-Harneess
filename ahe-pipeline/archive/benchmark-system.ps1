...
function Invoke-ScenarioTests {
    ...
    $ps=0;$scripts=@("self-improve.ps1","benchmark-system.ps1","agent-debugger.ps1","ahe-backup-rollback.ps1","verify-mcps.ps1");$sc=0;$scripts|%{if(Test-Path "$env:USERPROFILE\Scripts\$_"){$sc++}};$ps=[Math]::Floor($sc*3/$scripts.Count)
...
$medianScore = [math]::Round(($sortedScores[($sortedScores.Count - 1) / 2] + $sortedScores[$sortedScores.Count / 2]) / 2, 1)
